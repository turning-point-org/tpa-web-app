...
export default function PainPointsPage() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-semibold mb-4">Pain Point Research Interviews</h2>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {lifecycles.map((lifecycle) => {
            // Get total score and cost from pain points
            const score = calculateTotalScore(lifecycle.id);
            const cost = calculateTotalCost(lifecycle.id);
            
            // Get pain point count
            const painPointCount = painPointSummaries[lifecycle.id]?.pain_points?.length || 0;
            
            return (
              <div key={lifecycle.id} className="border rounded-lg shadow-sm bg-white p-6 flex flex-col justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-gray-800 mb-2">{lifecycle.name}</h3>
                  <p className="text-sm text-gray-600 mb-4">{lifecycle.description}</p>
                  <div className="mb-4 flex flex-wrap gap-2">
                    {/* Display pain point count */}
                    {painPointCount > 0 && (
                    ...
                    {/* Display cost from pain points */}
                    {cost > 0 && (
                      <Tag className="bg-[#7A2BF7] text-white">
                        {formatCurrency(cost)}
                      </Tag>
                    )}
                  </div>
                ...
                <div className="mt-auto">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white focus:ring-blue-500 focus:border-blue-500 text-sm"
                  >
